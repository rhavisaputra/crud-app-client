...
export default class postgresqlUser2 extends React.Component {
	constructor(props) {
		super(props)
		this.state = {
			isLoading: false,
	...
	handleDataUser() {
		axios.get(`${Config.BACKEND_HOST}/postgresql/user/all`, {
			headers: {
	...
	handleCurrentDataUser(userId) {
		axios.get(`${Config.BACKEND_HOST}/postgresql/user/${userId}`, {
			headers: {
				'Content-type': 'application/json'
			}
		}).then(result => {
	...
	handleCreate() {
        ...
        axios.post(`${Config.BACKEND_HOST}/postgresql/user`, data, {
			headers: {
				'Content-type': 'application/json'
			}
		}).then(result => {
    ...
    handleUpdate() {
        ...
        axios.put(`${Config.BACKEND_HOST}/postgresql/user/${userId}`, data, {
			headers: {
				'Content-type': 'application/json'
			}
		}).then(result => {
	...
	handleDelete(userId) {
		axios.delete(`${Config.BACKEND_HOST}/postgresql/user/${userId}`, {
			headers: {
				'Content-type': 'application/json'
			}
		}).then(result => {
